...
@section('content')
<div class="bg-white rounded-2xl shadow-sm border border-parchment-200 overflow-hidden">
    <div class="md:flex">
        {{-- Cover --}}
        ...
        <div class="md:w-7/12 p-8 lg:p-10">
            ...
            <div class="mt-6">
                ...
                <p class="text-ink-400 mt-2 leading-relaxed">{{ $book->description }}</p>
            </div>
                        ...
                        <div class="flex items-center gap-4 flex-wrap">
                            <div>
                                ...
                                <input type="number" name="quantity" value="1" min="1"
                                       ...
                                       class="w-24 px-4 py-2.5 border border-parchment-300 rounded-lg focus:ring-2 focus:ring-gold-500 focus:border-gold-500">
                            </div>
...
<div class="mt-10">
    ...
    @forelse($book->reviews as $review)
        <div class="bg-white rounded-2xl shadow-sm border border-parchment-200 p-6 mb-4">
                ...
                @auth
                    @if(auth()->id() === $review->user_id || auth()->user()->isAdmin())
                        <form action="{{ route('reviews.destroy', $review) }}" method="POST" class="flex-shrink-0">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-parchment-400 hover:text-red-500 transition-colors p-1" title="Delete review">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                </svg>
                            </button>
