...
@section('header')
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-3xl font-bold text-gray-900">Audit Log Detail</h2>
            <p class="text-gray-600 mt-1">Event: {{ $auditLog->event }}</p>
        </div>
        <a href="{{ route('admin.audit.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors text-sm font-medium">
            Back to Logs
        </a>
    </div>
@endsection
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <dl class="space-y-3">
                <div class="flex justify-between">
                    ...
                    <dd class="text-sm font-medium text-gray-900">{{ $auditLog->event }}</dd>
                </div>
                <div class="flex justify-between">
                    <dt class="text-sm text-gray-500">User</dt>
                    <dd class="text-sm font-medium text-gray-900">{{ $auditLog->user->name ?? 'System' }} (ID: {{ $auditLog->user_id ?? 'N/A' }})</dd>
                </div>
                <div class="flex justify-between">
                    <dt class="text-sm text-gray-500">Model</dt>
                    <dd class="text-sm font-medium text-gray-900">{{ $auditLog->auditable_type }} #{{ $auditLog->auditable_id }}</dd>
                </div>
                <div class="flex justify-between">
                    <dt class="text-sm text-gray-500">Timestamp</dt>
                    <dd class="text-sm font-medium text-gray-900">{{ $auditLog->created_at->format('Y-m-d H:i:s') }}</dd>
                </div>
                <div class="flex justify-between">
                    <dt class="text-sm text-gray-500">Checksum</dt>
                    <dd class="text-sm font-mono text-gray-600 break-all max-w-xs">{{ $auditLog->checksum ?? 'N/A' }}</dd>
                </div>
            </dl>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <dl class="space-y-3">
                ...
                <div class="flex justify-between">
                    ...
                    <dd class="text-sm text-gray-600 break-all max-w-sm">{{ $auditLog->url ?? 'N/A' }}</dd>
                </div>
                <div class="flex justify-between">
                    <dt class="text-sm text-gray-500">IP Address</dt>
                    <dd class="text-sm font-mono text-gray-900">{{ $auditLog->ip_address ?? 'N/A' }}</dd>
                </div>
                <div class="flex justify-between">
                    <dt class="text-sm text-gray-500">User Agent</dt>
                    <dd class="text-sm text-gray-600 break-all max-w-sm">{{ $auditLog->user_agent ?? 'N/A' }}</dd>
                </div>
            </dl>
        </div>
    </div>
