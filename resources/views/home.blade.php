...
@section('content')
    {{-- Hero Section --}}
    <div class="relative overflow-hidden mb-14 shadow-xl min-h-[520px] bg-ink-900">
        @if($heroBook?->cover_image)
            <img src="{{ $heroBook->cover_url }}" alt="{{ $heroBook->title }}" class="absolute inset-0 h-full w-full object-cover opacity-35">
        @endif
        <div class="absolute inset-0 bg-ink-950/65"></div>
        <div class="relative px-8 py-16 md:py-24 lg:py-28">
            <div class="max-w-3xl animate-fade-in-up">
                <span class="inline-flex items-center px-4 py-1.5 rounded-lg text-sm font-medium bg-gold-500/20 text-gold-300 border border-gold-500/30 mb-6">Welcome to PageTurner</span>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            @foreach($categories as $category)
                <a href="{{ route('categories.show', $category) }}" class="group">
                    <div class="bg-white rounded-lg p-6 shadow-sm hover:shadow-lg transition-all duration-300 text-center border border-parchment-200 hover:border-gold-400 transform hover:-translate-y-1 h-full flex flex-col items-center justify-center">
                        <div class="w-14 h-14 bg-parchment-100 rounded-lg flex items-center justify-center mb-3 group-hover:bg-gold-100 transition-colors">
